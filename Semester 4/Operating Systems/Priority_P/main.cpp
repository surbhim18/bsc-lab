...
void scheduleProcesses(Process *p, int n)
{
    ...
    while(pInQue!=n)
    {
        priorityProcess = que->dequeue();
        int index = findProcessIndex(p,priorityProcess,n);
        //cout<<"\nPoint 2\n";
        //cout<<"\nIndex: "<<index<<endl;
...
void display_Priority_P_Schedule(Process *p, int n)
{
    cout<<"Process\tBurst Time\tPriority\tArrival Time\tStarting Time\tFinishing Time\tWaiting Time\n";
...
int main()
{
    ...
    display_Priority_P_Schedule(p,n);
    cout<<endl;
